...
namespace TiendaZapateria.Server.Repositorio.Implementacion
{
    public class AutorizacionRepositorio : IAutorizacionRepositorio
    {
        ...
        private async Task<AutorizacionResponse> GuardarHistorialRefreshToken(int idUsuario, int idRol, string token, string refreshToken, string nombreRol)
        {
            ...
            return new AutorizacionResponse { IdUsuario = idUsuario, IdRol = idRol, Token = token, RefreshToken = refreshToken, Resultado = true, Msg = "Ok", nombreRol = nombreRol };
        }
        ...
        public async Task<AutorizacionResponse> DevolverToken(AutorizacionRequest autorizacion)
        {
            ...
            if (usuarioEncontrado != null && BCrypt.Net.BCrypt.Verify(autorizacion.ClaveAcceso, usuarioEncontrado.ClaveAcceso))
            {
                ...
                int idRol = await ObtenerIdRol(usuarioEncontrado.IdUsuario); // Aquí obtenemos el IdRol
                ...
                return await GuardarHistorialRefreshToken(idUsuario, idRol, tokenCreado, refreshTokenCreado, nombreRol);
            }
            else
            {
        ...
        public async Task<AutorizacionResponse> DevolverRefreshToken(RefreshTokenRequest refreshTokenRequest, int idUsuario, string nombreRol)
        {
            ...
            int idRol = await ObtenerIdRol(idUsuario); // Aquí obtenemos el IdRol
        ...
        public async Task<int> ObtenerIdRol(int idUsuario)
        {
            var usuarioConRol = await _dbContext.TblUsuarios
