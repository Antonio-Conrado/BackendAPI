...
namespace TiendaZapateria.Server.Controllers
{
    ...
    public class VistaController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("VistasSegunRol/{idRol}")]

        public async Task<IActionResult> ObtenerVistasPorRol(int idRol)
        {
            var responseApi = new ResponseAPI<List<VistaDTO>>();

            try
            {
                var rolExistente = await _dbContext.TblRols.AnyAsync(r => r.IdRol == idRol);
                if (!rolExistente)
                {
                    responseApi.EsCorrecto = false;
                ...
                var vistasPorRol = await _dbContext.TblVista
                    .Where(v => v.IdRol == idRol)
                    .ToListAsync();

                var vistasDTO = _mapper.Map<List<VistaDTO>>(vistasPorRol);

                responseApi.EsCorrecto = true;
                responseApi.Valor = vistasDTO;
                responseApi.Mensaje = "Vistas asociadas al rol cargadas correctamente";
            }
            catch (Exception ex)
            {
                responseApi.EsCorrecto = false;
